...
- **Active week:** W3 (D4 complete)
- **Streak:** 3 weeks
- **Cumulative problems solved:** 7
- **Last `/sync`:** 2026-05-24

## Next session
**First action:** W3 Day 5 — open cold with LC#3 s[l] over-shrink question (must score 2 before acceptance test begins). Acceptance test: re-run LC#121 and LC#3 on LeetCode to confirm fixed solutions still pass. Then retrospective.
**Retrieval question:** In LC#3's hashset version, why does `while (set.contains(s[l]))` over-shrink the window? What property of the set guarantees `set.contains(s[l])` is always true while the window is non-empty?
**Carry-forwards:** W3 P1 s[l] over-shrink WHY — three sessions at 1.5, closing score 1.5 today (mechanism correct, set-mirrors-window cause not explicitly stated; FLAG); `static_cast<int>(s.size())` must be applied without prompting — verify in D5 review.
