...
## Current state
- **Phase:** W4 — Binary search (in progress — D1+D2 complete)
- **Active week:** W4 (D1+D2 done; D3 next)
- **Streak:** 4 weeks completed + W4 in progress
- **Cumulative problems solved:** 11
- **Last `/sync`:** 2026-05-26

## Next session
**First action:** W4 D3 — Retrieval practice (0.5h). Three prompts closed-book: (1) W4 P1 loop invariant source of off-by-one; (2) LC#33 which-half-sorted logic + range check condition; (3) W3 P3 LC#3 `std::max` backward-jump invariant (score 1 carry-forward).
**Retrieval question:** In LC#875, as eating speed increases, what happens to total hours needed — does it increase, decrease, or could it go either way? Why does that property make binary search valid on the speed range [1, max_pile]?
**Carry-forwards:** `static_cast<int>(nums.size() - 1)` must appear without prompting at W4 D4 (4th recurrence — flag as blocking if triggered again); W3 P3 LC#3 `std::max` backward-jump (score 1 since W3 D3) must drill D3; binary search answer-space direction (score 1.5) resurface D3; LC#76 Minimum Window Substring still deferred.

## Today's artifact
2026-05-26 — W4 Day 2: LC#33 Accepted (196/196, 0ms). LC#875 Accepted (127/127, 10ms). Binary search on answer space pattern recognized before coding `[FIRST TIME]`. O(n log M) derived after probing (outer=log M, inner=O(n)). Answer-space direction: 1.5 (resurface D3). 4 bugs in LC#33 (r=size-1, else-if→else, inequality direction, return -1). long long overflow fix in LC#875. notes/w4_d2.md.
2026-05-26 — W4 Day 1: binary search intro from YTM bisection bridge. Loop invariant + loop condition + update rules derived. LC#704 Accepted (47/47, 0ms, 100% runtime). W3 P1 spaced retrieval: 2 (durable — complete causal chain cold). notes/w4_d1.md.
2026-05-25 — W3 Day 5: acceptance test (LC#121 212/212 Accepted, LC#3 988/988 Accepted). Retrospective: s[l] set-mirrors-window invariant closed at 2 [FIRST TIME] (four sessions); static_cast recurrence flagged (3rd time across 3 weeks); interview coding loop process change logged (D-006). W3 complete. notes/w3_d5.md.
2026-05-24 — W3 Day 4: code review. LC#3 fixes: signed/unsigned + `map`→`char_pos`. LC#121 fixes: signed/unsigned + `max`→`max_profit`. STL fluency: .count/.find/.contains answered. Retrieval: W3 P1 s[l] over-shrink=1.5 open/1.5 close (mechanism correct, WHY implicit — FLAG); W1 LC#11 pointer direction=1.5 open → 2 close. notes/w3_d4.md.
...
## Wins
- **2026-05-25** — LC#3 s[l] over-shrink invariant: "set mirrors window exactly — so s[l] by definition is in the set; contains(s[l]) is always true while non-empty" — stated cold, no scaffolding. Four sessions at 1.5 before closing. `[FIRST TIME]`
...
| 3 | Sliding window | complete | 2026-05-22 | 2026-05-25 | LC#3, LC#121 | LC#76 deferred. static_cast<int>(s.size()) recurring (3rd time). Interview coding loop process change self-identified. |
| 4 | Binary search (M1) | in progress | 2026-05-26 | — | LC#704, LC#33, LC#875 | D1+D2 complete. YTM bisection bridge: solid. Loop invariant derived clean. LC#33: 4 bugs (r=size-1 recurring — 4th week; else-if→else; inequality direction; return -1). LC#875: binary search on answer space pattern recognized `[FIRST TIME]`; long long overflow. Answer-space direction: 1.5 (resurface D3). static_cast recurring carry-forward. |
...
| 3 | 2 | 2 | 1 | W3 D3 open: LC#3 s[l] mechanism=1.5 (condition correct, mechanism wrong — said "stalls" when loop actually over-shrinks; s[l] always in set); LC#15 inner-skip=2 `[FIRST TIME]` (both conditions + why while loop clean — four sessions to close); LC#3 std::max backward-jump=1 (identified wrong value after two redirects, invariant named after prompt). W3 D3 close: LC#3 s[l] mechanism=1.5 (same error — "stalls / pointers cannot move"; session trace got user to self-state "b is duplicated" but over-shrink framing did not persist; FLAG — must open D4 cold before code review). W3 D4 open: LC#3 s[l] mechanism=1.5 (traced first two loop steps correctly; said "no sliding window — single pointer systematically eliminating chars" — mechanism observed but window invariant WHY not stated; also incorrectly said "c is not in set at that moment" when set={c}; full explanation given with window invariant + [a,b,c] trace). W3 D4 close: LC#3 s[l] mechanism=1.5 (both sides correct — "s[l] always true while non-empty, s[r] stops at collision" — but WHY s[l] always true still implicit; cause "set mirrors window exactly" not explicitly stated; FLAG for D5 open — must close cold). W3 D5 open: LC#3 s[l] =2 `[FIRST TIME]` (stated "set mirrors window exactly" → "s[l] by definition in set" → "always true while non-empty" — complete causal chain, cold; four sessions to close). W3 D5 close: LC#3 s[l] =2 (consistent with open — same answer, durable at close; real test is W4 D1 open). W4 D1 open: LC#3 s[l] =2 (durable confirmed — "set mirrors window exactly → s[l] by definition in set → always true while non-empty → loop only stops when set empty"; complete causal chain, cold, one week gap). W4 D1 close: 2 (identical to open — fully durable; three consecutive 2s: W3 D5 close, W4 D1 open, W4 D1 close — considered closed). |
| 4 | — | — | 1.5 | W4 D1 closing (loop invariant): 2 ("target is within mathematical range l ≤ t ≤ r" — clean, first attempt). W4 D2 closing (binary search on answer space — feasibility direction): 1.5 (said "ascending monotonicity" but direction wrong — as speed increases, hours decrease; step function structure not articulated; resurface D3). W4 D1 closing (W3 P1 spaced): 2 (three consecutive 2s — closed). |
| 5 | — | — | — | — |
| 6 | — | — | — | — |
| 7 | — | — | — | — |
| 8 | — | — | — | — |
