...
## Current state
- **Phase:** W4 — Binary search (starting)
- **Active week:** W4 (not started)
- **Streak:** 4 weeks
- **Cumulative problems solved:** 8
- **Last `/sync`:** 2026-05-25

## Next session
**First action:** W4 D1 — Binary search pattern intro from first principles. Before session, re-read the YTM bisection solver from cpp-finance-zero-to-hero. No keyboard until loop invariant (`l ≤ r` vs `l < r`) and mid calculation are traced on paper.
**Retrieval question:** In LC#3's hashset version, why does `while (set.contains(s[l]))` over-shrink the window? What property of the set guarantees `set.contains(s[l])` is always true while the window is non-empty? (Spaced W3 P1 — first close today; confirm it persists.)
**Carry-forwards:** `static_cast<int>(s.size())` must appear without prompting in W4 D4; LC#76 Minimum Window Substring (Hard) deferred; W3 P3 (std::max backward-jump, score 1) not retested — resurface W4 D3.
